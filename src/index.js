...
//database.cinemas.forEach(cinema => new Cinema(cinema).save())
const ACTION_TYPE = {
  TOGGLE_FAV_FILM: 'tff',
  SHOW_CINEMAS: 'sc',
...
bot.onText(/\/f(.+)/, (msg, [source, match]) => {
  ...
  Promise.all([
    ...
    const favText = isFav ? 'Удалить из избранного' : 'Добавить в избранного'
...
bot.onText(/\/c(.+)/, (msg, [source, match]) => {
  ...
  Cinema.findOne({uuid: cinemaUuid}).then(cinema => {
    bot.sendMessage(chatId, `Кинотеарт: ${cinema.name}`, {
      reply_markup: {
        inline_keyboard: [
          [{
            text: cinema.name,
